...
export const weekAnalysisPrompt = (postsData: any, user?: UserData) => {
  return `Я хочу, чтобы ты выступил в роли профессионального психолога и эмоционального коуча.

Ниже я передам тебе краткий эмоциональный и деятельностный контекст пользователя за последние 7 дней (или меньше). Данные включают:
	•	дату
	•	его настроение
...
Форматируй ответ 
Твой ответ должен содержать максимум 500 символов.
${
  user?.communication_style || 'Дуржелюбный'
}, как если бы писал письмо другу, а не формальный отчет.

Вот данные:
${JSON.stringify(postsData, null, 2)}

Если данных меньше чем за 7 дней — учитывай это в анализе.`;
};

export const questionPrompt = (postsData: any, user: UserData | null) => {
...
Ниже также представлен эмоциональный контекст пользователя за последние 7 дней (или меньше), включающий:
	•	дату
	•	настроение
	•	основной вид деятельности
	•	любимый цвет (по выбору)
...
Твой ответ должен содержать максимум 200 символов.
⸻
